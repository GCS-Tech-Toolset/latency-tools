 ...
 * File: LatencyPlotterEntryPoint.java
 */
...
public class LatencyPlotterEntryPoint {
    ...
    public void initCli(String[] args_) {
        try {
            ...
            if (StringUtils.isEmpty(_props.getInputFile())) {
                log.error("input file required, please use -f option");
                System.exit(1);
            }

            if (StringUtils.isEmpty(_props.getOutputFile())) {
                String base = FilenameUtils.getBaseName(_props.getInputFile());
                ...
                log.debug("output file:{}", _props.getOutputFile());
            }
            ...
            if (line.hasOption("t")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("title not specified, setting to:{}", _props.getTitle());
                }
            }
            ...
            if (line.hasOption("m")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("max-violation set to:{}", new DecimalFormat("#,###").format(_props.getInputProps().getMaxDataPointValue()));
                }
            }
            ...
            if (line.hasOption("s")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("sample-size:{}", new DecimalFormat("#,###").format(_props.getInputProps().getSampleSize()));
                }
            }

            if (line.hasOption("bucketcnt")) {
                _props.getOutputProps().setFrequencyBucketCount(Integer.parseInt(line.getOptionValue("bucketcnt")));
                if (log.isDebugEnabled()) {
                    log.debug("freqmap-bucket-size:{}", new DecimalFormat("#,###").format(_props.getOutputProps().getFrequencyBucketCount()));
                }
            }
            ...
            if (line.hasOption("processors")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("data-processors:{}", new DecimalFormat("#,###").format(_props.getInputProps().getReaderThreads()));
                }
            }
            ...
            if (line.hasOption("stddev")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("stddev-enabled:{}", _props.getOutputProps().isIncludeStdDev());
                }
            }
            ...
            if (line.hasOption("writepoint")) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("write-point:{}", _props.getOutputProps().isIncludeStdDev());
                }
            }
            ...
            if (line.hasOption("prewritewarmup")) {
                ...
                if (preWarmupCnt <= 0) {
                    throw new RuntimeException("Bad params [CLI], writewarmup must be >=1");
                }

                _props.getInputProps().setPreWriteWarmupCount(preWarmupCnt);
                if (log.isDebugEnabled()) {
                    log.debug("write-point-warmup:{}", _props.getOutputProps().isIncludeStdDev());
                }
            }
            ...
            if (line.hasOption("postwritewarmup")) {
                ...
                if (postWarmupCnt <= 0) {
                    ...
                    throw new RuntimeException("Bad params [CLI], post-warmup-count must not be greater than the warmup count");
                }
            ...
            if (line.hasOption("showheaders")) {
                try {
                    ...
                    _props.getOutputProps().setShowHeadersOnTerminal(showHeaders);
                } catch (Exception ex_) {
    ...
    protected ExtractedData extractData() {
        ...
        // return empty set
        //
        log.error("error occured while plotting data");
    ...
    private ILatencyStatsWriter createWriter(String type_) throws ConfigurationException {
        if ("xlsx".equalsIgnoreCase(type_)) {
            ...
            log.error("Unkonw/unsupported type:{}", type_);
